 ...
 */
@RuntimePermissions
open class BasePermissionsActivity : BaseSwipeBackActivity() {
    ...
    open fun openLocationTip(){
        Toast.makeText(this, "您已禁止了定位权限，请在应用权限设置内开启", Toast.LENGTH_SHORT).show()
    }
    ...
    open fun callPhoneRefuseTip(){
        Toast.makeText(this, "您已禁止了拨打电话权限，请在应用权限设置内开启", Toast.LENGTH_SHORT).show()
    }
...
//    }
    ...
    open fun takePhotoRefuseTip(){
        ...
        if(grantResults[0] != 0){
            Toast.makeText(this, "您已禁止了相机权限，请在应用权限设置内开启", Toast.LENGTH_SHORT).show()
        }
        if(grantResults.size >1 && grantResults[1] != 0){
            Toast.makeText(this, "您已禁止了写入权限，请在应用权限设置内开启", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    open fun selectPhoto() {

    }

    @OnNeverAskAgain(Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE)
    open fun selectPhotoRefuseTip(){

        if(grantResults[0] != 0){
            Toast.makeText(this, "您已禁止了读取权限，请在应用权限设置内开启", Toast.LENGTH_SHORT).show()
        }
        if(grantResults.size >1 && grantResults[1] != 0){
            Toast.makeText(this, "您已禁止了写入权限，请在应用权限设置内开启", Toast.LENGTH_SHORT).show()
        }
